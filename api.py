...
# # ══════════════════════════════════════════════════════════════════════════════
# #  GPU ENFORCEMENT — forces Ollama to load model into VRAM on every call
# #  RTX 3050 Laptop (4 GB VRAM) fits gemma3:4b fully — num_gpu=999 offloads
# #  all layers to GPU so EVERY question (Manglish or English) runs on GPU.
# # ══════════════════════════════════════════════════════════════════════════════
 
# # Set CUDA_VISIBLE_DEVICES so Ollama always picks GPU 0
...
#     corrupt users.json / tokens.json / shop_faq.json, and a concurrent
...
#     """Load model into GPU VRAM at startup (generous timeout for cold load)."""
#     ollama_client.warm_up(timeout=120)


# def _ollama_keepalive() -> None:
#     ollama_client.keepalive_loop(interval=240)
...
#         log.info("🖥️   GPU detected: %s (%d MiB VRAM) — Ollama will use GPU for ALL queries", gpu_name, total_mib)
#     else:
#         log.warning("⚠️   No CUDA GPU detected — Ollama will run on CPU (slow!)")
...
#             "ollama_num_gpu":  _OLLAMA_BASE_OPTIONS["num_gpu"],
...
#             "shop":               sum(len(v) for v in FAQS_SHOP.values()),
...
# ) -> dict:
#     _shop_flat: list[dict] = [faq for faqs in FAQS_SHOP.values() for faq in faqs]
#     pool_map: dict[str, list] = {
#         "all":             FAQS_SENTIMENT + FAQS_MANGLISH + FAQS_ENGLISH + _shop_flat,
...
#             faq_id = faq.get("id", "")
#         else:
#             first_q = faq.get("question") or faq.get("q") or ""
#             first_a = faq.get("answer") or faq.get("a") or ""
#             faq_id  = faq.get("id", "")
...
#         # FIX: detect_items_from_tables() now returns (items,
#         # extra_facts) -- see fix_pdf_dropped_facts.py. Unpack both
#         # here so promotional/programme facts (loyalty cards, offer
#         # codes, happy-hour deals) reach shop_info instead of being
#         # silently dropped, and so table_items stays a plain list
#         # (not a tuple) for the "table_items + regex_items" concat
#         # below.
...
#         # LLM FALLBACK for unstructured PDFs (prose menus, flyers, non-tabular
#         # layouts). If regex+table extraction is thin, ask Ollama to read the
#         # raw text and pull structured items. Only fires when needed.
#         if len(table_items) + len(regex_items) < 5:
#             try:
#                 from pdf import llm_extract_items
#                 _llm_items = llm_extract_items(
#                     full_text, auto_meta.get('shop_type', 'general'), 5)
#                 if _llm_items:
#                     log.info('[extract-pdf] LLM fallback added %d items', len(_llm_items))
#                     regex_items = regex_items + _llm_items
#             except Exception as _le:
#                 log.warning('[extract-pdf] LLM fallback failed: %s', _le)
...
#                     options=_OLLAMA_BASE_OPTIONS,   # ← GPU options applied here too
#                 )
...
#                     _slug_for_thread = slug_norm
#                     def _auto_populate(_s=_slug_for_thread):
#                         try:
#                             from populate_faq_from_pdf import run as _pop_run
#                             _pop_run(_s, dry_run=False)
#                             log.info("[extract-pdf] auto-populate done slug=%s", _s)
#                         except Exception as _pe:
#                             log.warning("[extract-pdf] auto-populate error: %s", _pe)
#                     _threading.Thread(target=_auto_populate, daemon=True).start()
#                 except Exception as _te:
#                     log.warning("[extract-pdf] auto-populate thread failed: %s", _te)
 
#             except Exception as e:
#                 log.warning("[extract-pdf] shop_context save failed: %s", e)
 
#         # EXTRACTION QUALITY GATE — fail loud, not silent. If the PDF yielded
#         # almost nothing (scanned image, unreadable layout), tell the owner
#         # instead of silently serving an empty bot.
#         _item_count = len(shop_info.get("shop_items", []))
#         _quality_warnings = []
#         if _item_count == 0:
#             _quality_warnings.append(
#                 "No items/services could be extracted. If this is a scanned or "
#                 "image-based PDF, please upload a text-based PDF, or list "
#                 "services as 'Name - Price' lines.")
#         elif _item_count < 3:
#             _quality_warnings.append(
#                 f"Only {_item_count} item(s) extracted — the PDF may be "
#                 "unstructured. Please review before going live.")
#         if not shop_info.get("shop_name"):
#             _quality_warnings.append("Shop name could not be detected.")

#         return JSONResponse({
#             "success":   _item_count > 0,
#             "extraction_quality": "poor" if _quality_warnings else "good",
...
#             # NOTE: deliberately NOT writing root shop_config.json here.
#             # Writing it made the global identity become whoever generated
#             # last (e.g. a dental shop showing the previous beauty bot's
#             # name). Each shop is isolated via shops/{slug}/shop_config.json
#             # and the pipeline reads per-slug. No shared global identity.
...
# @app.get("/shops", summary="List all registered shops (multi-tenant dashboard)")
# def get_all_shops() -> dict:
#     shops = list_shops()
#     return {"count": len(shops), "shops": shops}
 
 
# @app.get("/shops/{slug}", summary="Get shop summary by slug")
# def get_shop_summary(slug: str) -> dict:
#     slug = normalize_slug(slug)
#     ctx  = get_shop_context(slug)
#     return ctx.summary()
 
 
# @app.post("/shops/{slug}/reload", summary="Force reload shop context from disk")
# def force_reload_shop(slug: str) -> dict:
#     slug = normalize_slug(slug)
#     ctx  = reload_shop(slug)
#     return {"status": "ok", "slug": slug, **ctx.summary()}
...
async def extract_pdf_endpoint(
    ...
    try:
        ...
        # PERMANENT FIX: LLM fallback for unstructured/prose PDFs. Table+regex
        # extraction can legitimately find very few items when a shop owner
        # uploads a flyer or plain-text menu instead of a clean table --
        # without this, those shops silently get almost no shop_items at all.
        if len(table_items) + len(regex_items) < 5:
            try:
                from pdf import llm_extract_items
                _llm_items = llm_extract_items(
        ...
        REQUIRED = [
            "shop_name", "shop_type", "location", "city",
            "phone", "whatsapp", "hours_weekdays", "payment",
            "delivery_free", "delivery_charge", "delivery_areas",
        ]
        missing_fields = [
            f for f in REQUIRED
            if not shop_info.get(f) or shop_info[f] in ("", [], {})
            ...
            try:
                ...
                try:
                    ...
                    def _auto_populate(_s=_slug_for_thread):
                        try:
                            from populate_faq_from_pdf import run as _pop_run
                            _pop_run(_s, dry_run=False, use_rag_flag=True)
                            log.info("[extract-pdf] auto-populate done slug=%s", _s)
                        except Exception as _pe:
                            log.warning("[extract-pdf] auto-populate error: %s", _pe)
                        # Auto-expand question variants right after population so
                        # every shop gets full Manglish phrasing coverage WITHOUT
                        # a manual `python add_variants.py` step. More variants ->
                        # more queries hit a clean pre-written FAQ answer instead
                        # of falling to live (garble-prone) model generation.
                        try:
                            from add_variants import run as _var_run
                            _var_run(_s, dry_run=False)
                            log.info("[extract-pdf] auto-variants done slug=%s", _s)
                        except Exception as _ve:
                            log.warning("[extract-pdf] auto-variants error: %s", _ve)
                    _threading.Thread(target=_auto_populate, daemon=True).start()
                except Exception as _te:
                    log.warning("[extract-pdf] auto-populate thread failed: %s", _te)
...
async def generate_shop(
    ...
    try:
        ...
        # PERMANENT FIX: if this request has no shop_items (e.g. a later
        # call just to update hours/contact), do NOT overwrite a real menu
        # already saved by /admin/extract-pdf with an empty/dummy item.
        # Load whatever is already on disk for this shop and carry it
        # forward instead.
        if not info["shop_items"]:
            _existing_items_path = os.path.join("shops", slug, "shop_items.json")
            if os.path.exists(_existing_items_path):
                try:
                    with open(_existing_items_path, "r", encoding="utf-8-sig") as _f:
                        _existing_items = json.load(_f)
                    if _existing_items:
                        info["shop_items"] = _existing_items
                        log.info(
                            "[generate-shop] no shop_items in request -- "
                            "preserving %d existing items from %s",
                            len(_existing_items), _existing_items_path,
                        )
                except Exception as _e:
                    log.warning("[generate-shop] could not load existing shop_items.json: %s", _e)

        # Authoritative shop_type: explicit request value first, else the type
        # pdf.py detected from the FULL PDF (in shop_context.json). Stops
        # Ollama item-name inference hallucinating a wrong type
        # (beauty PDF -> dental_clinic -> 319 dental FAQs).
        _stype = getattr(req, "shop_type", None)
        if not _stype:
            try:
                _ctx_p = os.path.join("shops", normalize_slug(slug), "shop_context.json")
                with open(_ctx_p, "r", encoding="utf-8-sig") as _cf:
                    _stype = (json.load(_cf) or {}).get("shop_type")
            except Exception:
                _stype = None
        if _stype and _stype != "general":
            info["shop_type"] = _stype
            log.info("[generate-shop] using detected shop_type=%s", _stype)
...
def widget_script(request: Request, slug: str) -> str:
...
def widget_preview(request: Request, slug: str) -> str:
    base_url = str(request.base_url).rstrip("/")
    slug = normalize_slug(slug)
